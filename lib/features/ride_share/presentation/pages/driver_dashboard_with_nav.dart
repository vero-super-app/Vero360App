...
class _MarketplaceMerchantDashboardState
  ...
  final TextEditingController _recentSalesSearchController =
  ...
  static const String _kMerchantGuidePrefKey =
      'marketplace_merchant_guide_v1_done';
  static const String _kMerchantGuideShowOnNextOpenKey =
  ...
  Future<void> _pullPhoneAndProfileFromFirestore() async {
    ...
    try {
      ...
      final phoneVal = _sanitizePhone((data['phone'] ?? '').toString().trim());
  ...
  Future<void> _fetchCurrentUserMe() async {
    ...
    try {
      ...
      if (resp.statusCode == 200) {
        ...
        final phoneVal =
            _sanitizePhone((user['phone'] ?? '').toString().trim());
        ...
        setState(() {
          ...
          _merchantProfileUrl = authPhoto.isNotEmpty ? authPhoto : picVal;
        });
      } else {
        ...
        if (!dashboardData.containsKey('error')) {
          ...
          setState(() {
            ...
            if (merchant is Map) {
              ...
              final mp =
                  _sanitizePhone((merchant['phone'] ?? '').toString().trim());
    ...
    try {
      ...
      final sold = orders.where((o) {
        ...
        if (o.status == OrderStatus.confirmed &&
            o.paymentStatus == PaymentStatus.paid) return true;
        return false;
      }).toList();
      ...
      final earnings =
          sold.fold<double>(0, (sum, o) => sum + o.total.toDouble());
      if (!mounted) return;
      setState(() {
        _soldItems = count;
  ...
  Future<void> _pickAndUploadProfile(ImageSource src) async {
    ...
    try {
      ...
      if (e.code == 'object-not-found' || (e.message ?? '').contains('404')) {
        _toastErr(
            'Upload failed. Check network and that the server is running.');
      } else {
  ...
  Future<String> _uploadProfileViaBackend(XFile file) async {
    ...
    if (resp.statusCode < 200 || resp.statusCode >= 300) {
      if (resp.statusCode == 404)
        throw Exception('Profile picture endpoint not found');
      if (resp.statusCode == 401)
        throw Exception('Session expired. Please sign in again.');
    ...
    final url =
        (data['profilepicture'] ?? data['profilePicture'] ?? data['url'])
            ?.toString();
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      bottomNavigationBar:
          widget.embeddedInMainNav ? null : _buildMerchantNavBar(),
    );
  }
  ...
  static const List<({String title, String body, IconData icon})>
      _merchantGuideSteps = [
    (
      title: 'ikani katundu wanu koyamba',
      body:
          'Dinani "Add Item" kuti muike katundu wanu. kenako ikani  name, price, photo and category.',
      icon: Icons.add_box_rounded
    ),
    (
      title: 'kulongosola katundu wanu',
      body:
          'Yuzani "My Items"  kt mu wone, musithe,kuika ndikuchosa katundu wanu pa msika.',
      icon: Icons.inventory_2_rounded
    ),
    (
      title: 'Tsegulani waleti yanu',
      body:
          'tsegulani Waleti yanu pa dashboard kuti muthe kulandila ma payments and ikani PIN kuti muteteze waleti yanu.',
      icon: Icons.account_balance_wallet_rounded
    ),
    (
      title: 'Mukagulisa',
      body:
          'chongani ma  orders as shipped or delivered. onani zomwe mwagulisa pa recent Sales  ndipo pangani makasitomala anu kukhala a tcheru.',
  ...
  Widget _buildDashboardContent() {
    ...
    if (!_merchantGuideCheckScheduled) {
      ...
      WidgetsBinding.instance
          .addPostFrameCallback((_) => _maybeShowMerchantGuide());
    }
  ...
  static final DateFormat _recentSaleDateFmt = DateFormat('dd MMM yyyy, HH:mm');
      ...
      children: [
        ...
        if (_recentSalesOrders.isEmpty)
          ...
          Container(
            ...
            child: Center(child: Text('No matches for "$query"')),
          )
        else
          ...displayList.map((o) {
                      ...
                      Row(
                        children: [
                          ...
                          IconButton(
                            ...
                            onPressed: () {
                              Clipboard.setData(
                                  ClipboardData(text: o.orderNumber));
                              ToastHelper.showCustomToast(
                                  context, 'Order number copied',
        ...
        child: SizedBox(
          ...
          child: Row(
            children: [
              ...
              Expanded(
                  child: _buildNavItem(Icons.shopping_cart_rounded, 'Cart', 2)),
              Expanded(
  ...
  Widget _buildNavItem(IconData icon, String label, int index) {
    ...
    return GestureDetector(
      ...
      child: AnimatedContainer(
        ...
        width: double.infinity,
        padding:
            EdgeInsets.symmetric(horizontal: hPad, vertical: narrow ? 8 : 10),
        alignment: Alignment.center,
        decoration: BoxDecoration(
          color:
