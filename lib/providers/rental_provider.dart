...
// Get car details
final carDetailsFutureProvider = FutureProvider.family<CarModel, int>((ref, carId) async {
  final service = ref.watch(carRentalServiceProvider);
  return service.getCarDetails(carId);
});

// Get user bookings
final userBookingsFutureProvider = FutureProvider<List<CarBookingModel>>((ref) async {
  final service = ref.watch(carRentalServiceProvider);
  return service.getUserBookings();
});

// Get specific booking
final bookingDetailsFutureProvider = FutureProvider.family<CarBookingModel, int>((ref, bookingId) async {
  final service = ref.watch(carRentalServiceProvider);
...
class BookingFormNotifier extends StateNotifier<BookingFormState> {
  ...
  void setPickupLocation(String location) => state = state.copyWith(pickupLocation: location);
